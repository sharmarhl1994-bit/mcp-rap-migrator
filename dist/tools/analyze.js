...
export async function analyzeModulePool(_adt, args) {
...
function extractFacts(src) {
    ...
    const tableMatches = [...src.matchAll(/(?:^TABLES\s+(\w+)|SELECT[^.]+FROM\s+(\w+))/gim)]
        ...
        .filter((t) => !!t && !["SCREEN", "SSCRFIELDS", "SYST"].includes(t));
    ...
    const reasons = [];
    if (pboModules.length > 10)
        reasons.push(`${pboModules.length} PBO modules`);
    if (functionCalls.length > 5)
        reasons.push(`${functionCalls.length} CALL FUNCTIONs`);
    if (src.includes("CALL TRANSACTION"))
        reasons.push("CALL TRANSACTION found");
    if (src.includes("AUTHORITY-CHECK"))
        reasons.push("AUTHORITY-CHECK found");
    if (src.includes("CALL SCREEN"))
        reasons.push("Dynamic screen calls found");
    return {
        ...
        complexity: (reasons.length === 0 ? "LOW" : reasons.length <= 3 ? "MEDIUM" : "HIGH"),
        reasons,
    };
}
function condenseForLLM(src, facts) {
    const lines = src.split("\n");
    const relevant = [];
    for (const line of lines) {
        const l = line.trim().toUpperCase();
        if (l.startsWith("MODULE") || l.startsWith("ENDMODULE") ||
...
async function runLLMAnalysis(programName, condensedCode, facts) {
