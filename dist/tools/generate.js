...
export async function generateRapSkeleton(args) {
    let analysis;
    let cdsMapping = {};
    try {
    ...
    if (args.cds_source_mapping) {
        try {
            cdsMapping = JSON.parse(args.cds_source_mapping);
        }
        catch { }
    }
...
${paiMapping.map((m) => `  ${m.pai_module} → ${m.rap_equivalent} (${m.notes})`).join("\n")}

PBO MODULE → RAP MAPPING:
${pboMapping.map((m) => `  ${m.pbo_module} → ${m.rap_equivalent} (${m.notes})`).join("\n")}
...
Generate valid ABAP/CDS code for all 4 objects. Return ONLY valid JSON (no markdown):
{
  ...
  "generation_notes": ["important things developer should know"]
    ...
    try {
        ...
        const code = parseJSON(result.text);
