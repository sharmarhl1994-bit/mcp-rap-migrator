...
function toTableArray(raw: any): string[] {
  if (!raw) return [];
  ...
  if (typeof raw === "object") {
    if (raw.tables) return toTableArray(raw.tables);
    return Object.values(raw).flatMap((v: any) => toTableArray(v));
  }
  return [];
}

export async function checkReleasedCds(_adt: any, args: any) {
  const tables = toTableArray(args?.tables ?? args);
  if (tables.length === 0) return { content:[{type:"text",text:JSON.stringify({error:"No tables"})}], isError:true };
...
[{"table":"VBAK","recommended":"I_SalesOrder","release_state":"C1","confidence":"HIGH","reason":"official API","use_raw_table":false}]`;
  ...
  for (const v of verdicts) mapping[v.table] = v.use_raw_table ? v.table : v.recommended;
  for (const t of tables) if (!mapping[t]) mapping[t] = t;

  return { content:[{ type:"text", text:JSON.stringify({
    "🔍 WHAT I GOT": { tables_checked: tables, tokens_used: tokens, backend: "SAP AI Core" },
    "📊 MY ANALYSIS": {
      summary: verdicts.map((v:any) => v.use_raw_table
        ? `⚠️ ${v.table} → raw table`
        : `✅ ${v.table} → ${v.recommended} (${v.release_state}, ${v.confidence})`),
      clean_core_score: `${verdicts.filter((v:any) => !v.use_raw_table).length}/${tables.length}`,
    },
