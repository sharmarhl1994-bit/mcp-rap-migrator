...
const ACTIVATION_ORDER: Record<string, number> = {
  DDLS: 1,
  BDEF: 2,
  CLAS: 3,
  INTF: 4,
...
export async function validateAndActivate(
  adt: AdtClient,
  args: { objects: Array<{ type: string; name: string }> }
) {
...
export async function createTransport(
  adt: AdtClient,
  args: { description: string; target_system?: string }
) {
  try {
    const transportNum = await adt.createTransportRequest(
      args.description,
      args.target_system
    );
    return {
