...
const server = new Server(
  { name: "mcp-rap-migrator", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    switch (name) {
      case "analyze_module_pool":   return await analyzeModulePool(adt, args as any);
      case "check_released_cds":    return await checkReleasedCds(adt, args as any);
      case "generate_rap_skeleton": return await generateRapSkeleton(args as any);
      case "human_checkpoint":      return await humanCheckpoint(args as any);
      case "write_abap_object":     return await writeAbapObject(adt, args as any);
      case "validate_and_activate": return await validateAndActivate(adt, args as any);
      case "create_transport":      return await createTransport(adt, args as any);
      default:
